...
public class TribeInfoActivity extends Activity {
                ...
                public void onClick(View v) {
                    mTribeService.joinTribe(new IWxCallback() {
                        ...
                        public void onError(int code, String info) {
                            ...
                            IMNotificationUtils.getInstance().showToast(TribeInfoActivity.this, "加入群失败, code = " + code + ", info = " + info);
                        }
    ...
    private void getTribeMembersFromServer() {
        mTribeService.getMembersFromServer(new IWxCallback() {
            ...
            @Override
            public void onError(int code, String info) {
                IMNotificationUtils.getInstance().showToast(TribeInfoActivity.this, "error, code = " + code + ", info = " + info);
            }
    ...
    private class ConfigAtAllCallback implements IWxCallback{
        ...
        public void onSuccess(Object... result) {
            uiHandler.post(new Runnable() {
                @Override
                public void run() {
                    IMNotificationUtils.getInstance().showToast(TribeInfoActivity.this, "设置成功：" + mTribe.isEnableAtAll());
                    if(mTribe.isEnableAtAll()) {
                        enableAtAllSwitch.setImageResource(R.drawable.on_switch);
                    } else {
                        enableAtAllSwitch.setImageResource(R.drawable.off_switch);
        ...
        public void onError(int code, String info) {
            uiHandler.post(new Runnable() {
                @Override
                public void run() {
                    IMNotificationUtils.getInstance().showToast(TribeInfoActivity.this, "设置失败：" + mTribe.isEnableAtAll());
                }
            });
        }
